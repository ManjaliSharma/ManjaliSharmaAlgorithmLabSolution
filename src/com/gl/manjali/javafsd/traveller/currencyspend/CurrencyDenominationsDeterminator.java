...
import com.gl.datastructure.common.DataStructureUtils;
...
public class CurrencyDenominationsDeterminator {
	...
	public void determine() {
		...
		while(index < denominations.length) {
			...
			if (quotient !=0) {
				...
				// 128 - 78 -> 50
				// 50 - 50 -> 0
				balancedOutPayAmount = balamcedOutPayAmount - (temp);
		...
		private void printErrorMessage() {
			...
			System.out.println("For the denomination values" + DataStructureUtils.printAndReturn(denominations) + ", it is not possible to give for the payment amount ["+ payAmount + "]");		}
	}
	...
	private void printSuccessMessage(int resultIndex) {
		...
		// Output -> Debominations = {a, c, [EMPTY], [EMPTY]} / 2
		...
		System.out.println("For the payment [" + payAmount + "] and for the denominations " + DataStructureUtils.printAndReturn(denominations) + ", the " + "payment approach can be used to give minimum number of notes");
	...
	private static void test(int[] denomination, int payAmount) {
		
		System.out.println("--------------------------------");
		CurrencyDenominationsDeterminator determinator = 
				new CurrencyDenominationsDeterminator(denomination, payAmount);
